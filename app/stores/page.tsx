  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50">Connected TikTok Shop Accounts</h2>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-2">
        {stores.map((store) => (
          <div
            key={store.id}
            className="flex flex-col justify-between rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-[#121620] p-6 shadow-xs hover:shadow-md transition-all duration-200"
          >
            <div>
              {/* Store Header */}
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3.5">
                  <span className="text-4xl drop-shadow-xs">{store.flag}</span>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-bold text-slate-900 dark:text-slate-50">{store.name}</h3>
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    title="Force Resync"
                    className="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 dark:border-slate-800 text-slate-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 hover:text-rose-600 dark:hover:text-rose-400 transition-colors"
                  >
              ...
              {/* Financial Metrics */}
              <div className="grid grid-cols-3 gap-3 mt-6 rounded-2xl bg-slate-50/70 dark:bg-[#0f1117]/60 p-3.5 border border-slate-100 dark:border-slate-800 font-mono-numeric">
                <div>
                  <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-500">Total Revenue</p>
                ...
                <div>
                  <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-500">Net Profit</p>
                  <p className="text-base font-black text-emerald-600 dark:text-[#4ade80] mt-0.5">
                    {formatCurrency(store.netProfit, store.currency)}
                  </p>
                </div>
                <div>
                  <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-500">Profit Margin</p>
                  <p className="text-base font-black text-amber-600 dark:text-[#fb923c] mt-0.5">
              ...
              <div className="mt-4 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                <div className="flex items-center gap-1.5 font-medium">
                  ...
                  <span>Access: </span>
                  <span className="font-bold text-slate-800 dark:text-slate-200">{store.accountRole}</span>
                </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedStoreId === store.id
                    ...
                    : 'border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5'
                )}
              >
      ...
      {/* Permissions Matrix */}
      <div className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-[#121620] p-6 shadow-xs">
        <div className="flex items-center gap-2 mb-4">
          <Users className="h-5 w-5 text-slate-700 dark:text-slate-300" />
          <h3 className="text-base font-bold text-slate-900 dark:text-slate-50">Staff User Roles & Permissions</h3>
        </div>
