...
export default function ExpensesPage() {
  const { filteredExpenses, addExpense, stores } = useStore();
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Cost & Operating Expense Tracking</h2>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white dark:bg-[#151b26] p-5 shadow-sm">
          <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">Total Fixed Monthly Expenses</p>
          <p className="text-2xl font-black text-gray-900 dark:text-white mt-1">{formatCurrency(totalMonthlyExpenses)}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Deducted from gross profit calculations</p>
        </div>
        <div className="rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white dark:bg-[#151b26] p-5 shadow-sm">
          <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">Active Staff & VA Payroll</p>
          <p className="text-2xl font-black text-purple-600 dark:text-purple-400 mt-1">{formatCurrency(1200.00)}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">2 Virtual Assistants</p>
        </div>
        <div className="rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white dark:bg-[#151b26] p-5 shadow-sm">
          <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">Software & Subscriptions</p>
          <p className="text-2xl font-black text-blue-600 dark:text-blue-400 mt-1">{formatCurrency(199.00)}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">TikTok tools, automation API</p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-gray-200/80 dark:border-gray-800 bg-white dark:bg-[#151b26] shadow-sm">
        <div className="p-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <h3 className="font-bold text-gray-900 dark:text-white text-sm">Operating Expenses Ledger</h3>
          <span className="text-xs font-medium text-gray-400 dark:text-gray-500">{filteredExpenses.length} Entries</span>
        </div>
                ...
                <tr key={exp.id} className="hover:bg-gray-50/80 dark:hover:bg-white/5 transition-colors">
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-2.5">
                      <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gray-100 dark:bg-[#0f1117]">
                        {getCategoryIcon(exp.category)}
                      </div>
                      <span className="font-bold text-gray-900 dark:text-white">{exp.title}</span>
                    </div>
                  </td>
                  <td className="py-3.5 px-3 font-semibold text-gray-600 dark:text-gray-400">{exp.category}</td>
                  <td className="py-3.5 px-3 text-gray-800 dark:text-gray-200">{exp.storeName}</td>
                  <td className="py-3.5 px-3">
                    <span className="rounded-md bg-gray-100 dark:bg-white/5 px-2 py-0.5 font-semibold text-gray-700 dark:text-gray-300">
                      {exp.recurrence}
                    </span>
                  </td>
                  <td className="py-3.5 px-3 text-gray-500 dark:text-gray-400">{exp.date}</td>
                  <td className="py-3.5 px-3 font-black text-gray-900 dark:text-white">{formatCurrency(exp.amount)}</td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-xs animate-in fade-in">
          <div className="w-full max-w-md rounded-3xl bg-white dark:bg-[#151b26] border border-gray-100 dark:border-gray-800 p-6 shadow-2xl space-y-4 text-gray-900 dark:text-white">
            <h3 className="text-base font-bold text-gray-900 dark:text-white">Add New Expense / Overhead</h3>

            <form onSubmit={handleAdd} className="space-y-3.5">
              <div>
                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Expense Description</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#0f1117] p-2.5 text-xs font-bold dark:text-white"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Recurrence</label>
                  <select
                    value={recurrence}
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Store Allocation</label>
                  <select
                    value={storeId}
              ...
              <div className="flex justify-end gap-2 pt-3">
                ...
                <button
                  type="submit"
                  className="rounded-xl bg-[#84cc16] px-5 py-2 text-xs font-bold text-black hover:bg-[#72b012]"
                >
                  Save Expense
                </button>
              </div>
