...
export default function NotificationsDrawer({ isOpen, onClose }: NotificationsDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="relative h-full w-full max-w-md bg-white dark:bg-[#121620] border-l border-slate-200 dark:border-slate-800 p-6 shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-300 text-slate-900 dark:text-white">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-slate-50">Notifications & AI Alerts</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">{insights.length} active updates</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="mt-5 space-y-3">
              ...
              <div
                ...
                className={cn(
                  ...
                  isWin
                    ...
                    : isScale
                    ? 'border-purple-200/80 dark:border-purple-900/50 bg-purple-50/50 dark:bg-purple-950/20'
                    : 'border-blue-200/80 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-950/20'
                )}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-900 dark:text-white">{insight.badge}</span>
                      <button
                    ...
                    <p className="mt-1 text-xs font-semibold text-slate-800 dark:text-slate-200">{insight.text}</p>
                    {insight.detail && <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">{insight.detail}</p>}
                  </div>
                </div>
              </div>
            );
          ...
          {/* System sync status */}
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#0f1117] p-4">
