...
const storeColors = ['#8b5cf6', '#3b82f6', '#10b981', '#f59e0b', '#ec4899'];

export default function RevenueByStoreChart() {
  const { stores, totalRevenue } = useStore();

  const chartData = stores.map((store, index) => ({
    name: store.name,
    value: store.totalRevenue,
    percentage: ((store.totalRevenue / totalRevenue) * 100).toFixed(1),
  ...
  return (
    <div className="flex h-full flex-col justify-between rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white dark:bg-[#151b26] p-5 shadow-sm transition-colors">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-gray-900 dark:text-white">Revenue by Store</h3>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 my-auto py-2">
        ...
        <div className="relative h-44 w-44 shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className="text-sm font-black text-gray-900 dark:text-white">{formatCurrency(totalRevenue)}</span>
          </div>
        </div>
